...
DygraphLayout.prototype._evaluateLineCharts = function () {
    ...
    for (var a = 0; a < this.datasets.length; ++a) {
        ...
        for (var f = 0; f < e.length; f++) {
            ...
            var g = DygraphLayout._calcYNormal(b, k);
            var l = {x:m, y:g, xval:d, yval:k, name:h};
            if (c && n[1] === null) {
                l.yval = null
            }
            this.points.push(l);
...
DygraphLayout.prototype.evaluateWithError = function () {
    ...
    for (var a = 0; a < this.datasets.length; ++a) {
        ...
        for (g = 0; g < f.length; g++, h++) {
            ...
            var m = parseFloat(o[1]);
            if (c == this.points[h].xval && m == this.points[h].yval) {
                var k = parseFloat(o[2]);
                var d = parseFloat(o[3]);
                var n = m - k;
                var b = m + d;
                this.points[h].y_top = DygraphLayout._calcYNormal(e, n);
                this.points[h].y_bottom = DygraphLayout._calcYNormal(e, b)
            }
        }
    }
};
...
DygraphCanvasRenderer.prototype._renderAnnotations = function () {
    ...
    for (var g = 0; g < m.length; g++) {
        var e = m[g];
        if (e.canvasx < this.area.x || e.canvasx > this.area.x + this.area.w || e.canvasy < this.area.y || e.canvasy > this.area.y + this.area.h) {
            continue
        }
        var k = e.annotation;
        var l = 6;
        ...
        k.div = c;
        this.dygraph_.addEvent(c, "click", j("clickHandler", "annotationClickHandler", e, this));
        this.dygraph_.addEvent(c, "mouseover", j("mouseOverHandler", "annotationMouseOverHandler", e, this));
        this.dygraph_.addEvent(c, "mouseout", j("mouseOutHandler", "annotationMouseOutHandler", e, this));
        this.dygraph_.addEvent(c, "dblclick", j("dblClickHandler", "annotationDblClickHandler", e, this));
...
DygraphCanvasRenderer.makeNextPointStep_ = function (b, c, a) {
    if (b) {
        return function (d) {
            while (++d < a) {
                if (!(c[d].yval === null)) {
...
DygraphCanvasRenderer.prototype._drawStyledLine = function (r, w, c, t, n, e, s, x, g) {
    var p = function (i) {
        return(i === null || isNaN(i))
    };
    var f = this.attr_("stepPlot");
    var z = this.layout.setPointsOffsets[w];
    var y = this.layout.setPointsLengths[w];
    var a = z + y;
    var v = this.layout.points;
    var d = null;
    var b = null;
    var k = [];
    if (!Dygraph.isArrayLike(e)) {
        e = null
    }
    var q;
    var o = DygraphCanvasRenderer.makeNextPointStep_(this.attr_("connectSeparatedPoints"), v, a);
    r.save();
    for (var u = z; u < a; u = o(u)) {
        q = v[u];
        if (p(q.canvasy)) {
            if (f && d !== null) {
                r.beginPath();
                r.strokeStyle = t;
                r.lineWidth = this.attr_("strokeWidth");
                this._dashedLine(r, d, b, q.canvasx, b, e);
                r.stroke()
            }
            d = b = null
        } else {
            var h = (!d && (u == v.length - 1 || p(v[u + 1].canvasy)));
            if (d === null) {
                d = q.canvasx;
                b = q.canvasy
            } else {
                if (Math.round(d) == Math.round(q.canvasx) && Math.round(b) == Math.round(q.canvasy)) {
                    continue
                }
                if (n) {
                    r.beginPath();
                    ...
                    this._dashedLine(r, d, b, q.canvasx, q.canvasy, e);
                    d = q.canvasx;
                    b = q.canvasy;
                    r.stroke()
                }
    ...
    for (var l = 0; l < k.length; l++) {
        var m = k[l];
        r.save();
        x(this.dygraph_, c, r, m[0], m[1], t, g);
        r.restore()
    }
    r.restore()
};
DygraphCanvasRenderer.prototype._drawLine = function (a, c) {
    var g = this.layout.setNames;
    var e = g[c];
    var f = this.dygraph_.attr_("strokeWidth", e);
    var b = this.dygraph_.attr_("strokeBorderWidth", e);
    var d = this.dygraph_.attr_("drawPointCallback", e) || Dygraph.Circles.DEFAULT;
    if (b && f) {
        this._drawStyledLine(a, c, e, this.dygraph_.attr_("strokeBorderColor", e), f + 2 * b, this.dygraph_.attr_("strokePattern", e), this.dygraph_.attr_("drawPoints", e), d, this.dygraph_.attr_("pointSize", e))
    }
    this._drawStyledLine(a, c, e, this.colors[e], f, this.dygraph_.attr_("strokePattern", e), this.dygraph_.attr_("drawPoints", e), d, this.dygraph_.attr_("pointSize", e))
};
DygraphCanvasRenderer.prototype._renderLineChart = function () {
    var t = this.elementContext;
    ...
    var n = this.attr_("stepPlot");
    var z = this.layout.points;
    var o = z.length;
    var u, A, x, k, g, h, v, l, q, F, b, C, e;
    var E = this.layout.setNames;
    var w = E.length;
    this.colors = {};
    for (A = 0; A < w; A++) {
        this.colors[E[A]] = this.colorScheme_[A % this.colorScheme_.length]
    }
    for (A = o; A--;) {
        u = z[A];
    ...
    if (D) {
        ...
        for (A = 0; A < w; A++) {
            l = E[A];
            e = this.dygraph_.axisPropertiesForSeries(l);
            v = this.colors[l];
            var G = this.layout.setPointsOffsets[A];
            var B = this.layout.setPointsLengths[A];
            var d = G + B;
            var s = DygraphCanvasRenderer.makeNextPointStep_(this.attr_("connectSeparatedPoints"), z, d);
            k = NaN;
            g = NaN;
            h = [-1, -1];
            C = e.yscale;
            b = new RGBColor(v);
            F = "rgba(" + b.r + "," + b.g + "," + b.b + "," + y + ")";
            t.fillStyle = F;
            t.beginPath();
            for (x = G; x < d; x = s(x)) {
                u = z[x];
                if (u.name == l) {
                    ...
                    if (n) {
                        q = [u.y_bottom, u.y_top];
                        g = u.y
                    } else {
                        q = [u.y_bottom, u.y_top]
                    }
                    q[0] = this.area.h * q[0] + this.area.y;
                    q[1] = this.area.h * q[1] + this.area.y;
                    if (!isNaN(k)) {
                        if (n) {
                            t.moveTo(k, q[0])
                        } else {
                        ...
                        t.lineTo(u.canvasx, q[0]);
                        t.lineTo(u.canvasx, q[1]);
                        if (n) {
                            t.lineTo(k, q[1])
                        } else {
                            t.lineTo(k, h[1])
                ...
                for (x = G; x < d; x = s(x)) {
                    ...
                    if (u.name == l) {
                        ...
                        if (f) {
                            ...
                            if (n) {
                                if (h[0] === -1) {
                                    p[u.canvasx] = [u.canvasy, c]
                                } else {
                                    p[u.canvasx] = [u.canvasy, h[0]]
                                }
                            } else {
                                p[u.canvasx] = u.canvasy
                            }
                        } else {
                            q = [u.canvasy, c]
                        }
                        if (!isNaN(k)) {
                            t.moveTo(k, h[0]);
                            if (n) {
                                t.lineTo(u.canvasx, h[0]);
                                if (a) {
                                    t.lineTo(u.canvasx, a[1])
                                } else {
                                    t.lineTo(u.canvasx, q[1])
                                }
                            } else {
...
Dygraph.prototype.__init__ = function (a, c, k) {
    ...
    for (var f = 0; f < Dygraph.PLUGINS.length; f++) {
        ...
        var e = new h();
        var j = {plugin:e, events:{}, options:{}, pluginOptions:{}};
...
Dygraph.prototype.attr_ = function (d, a) {
    var c = [];
    c.push(this.attrs_);
    if (this.user_attrs_) {
        c.push(this.user_attrs_);
        if (a) {
            if (this.user_attrs_.hasOwnProperty(a)) {
                c.push(this.user_attrs_[a])
            }
            if (a === this.highlightSet_ && this.user_attrs_.hasOwnProperty("highlightSeriesOpts")) {
                c.push(this.user_attrs_.highlightSeriesOpts)
            }
        }
    }
    var b = null;
...
Dygraph.prototype.getPropertiesForSeries = function (c) {
    ...
    return{name:c, column:a, visible:this.visibility()[a - 1], color:this.plotter_.colors[c], axis:1 + this.seriesToAxisMap_[c]}
};
...
Dygraph.prototype.doUnzoom_ = function () {
    ...
    if (c) {
        ...
        if (a) {
            ...
            for (g = 0; g < this.axes_.length; g++) {
                var e = this.axes_[g];
                h.push(e.valueRange != null ? e.valueRange : e.extremeRange)
            }
        }
        var j = this;
...
Dygraph.prototype.findClosestRow = function (e) {
    var d = Infinity;
    var b = -1;
    ...
    for (var f = 0; f < c; f++) {
        var a = g[f];
        if (!Dygraph.isValidPoint(a, true)) {
            continue
        }
        var h = Math.abs(a.canvasx - e);
        if (h < d) {
            d = h;
            b = f
        }
    }
    return this.idxToRow_(b)
};
Dygraph.prototype.findClosestPoint = function (f, e) {
    var m = Infinity;
    var n = -1;
    var o = this.layout_.points;
    var k, q, p, l, d, c;
    for (var b = 0; b < this.layout_.datasets.length; ++b) {
        var h = this.layout_.setPointsOffsets[b];
        var j = this.layout_.setPointsLengths[b];
        for (var g = 0; g < j; ++g) {
            var l = o[h + g];
    ...
    return{row:n + this.getLeftBoundary_(), seriesName:a, point:d}
};
Dygraph.prototype.findStackedPoint = function (j, i) {
    var f = this.findClosestRow(j);
    var p = this.getLeftBoundary_();
    var c = f - p;
    var n = this.layout_.points;
    var e, q;
    for (var m = 0; m < this.layout_.datasets.length; ++m) {
        var g = this.layout_.setPointsOffsets[m];
        var o = this.layout_.setPointsLengths[m];
        if (c >= o) {
            continue
        }
        var b = n[g + c];
        if (!Dygraph.isValidPoint(b)) {
            continue
        }
        var k = b.canvasy;
        if (j > b.canvasx && c + 1 < o) {
            var a = n[g + c + 1];
            if (Dygraph.isValidPoint(a)) {
                var h = a.canvasx - b.canvasx;
                if (h > 0) {
                    var l = (j - b.canvasx) / h;
                    k += l * (a.canvasy - b.canvasy)
                }
            }
        } else {
            if (j < b.canvasx && c > 0) {
                var d = n[g + c - 1];
                if (Dygraph.isValidPoint(d)) {
                    var h = b.canvasx - d.canvasx;
                    if (h > 0) {
                        var l = (b.canvasx - j) / h;
                        k += l * (d.canvasy - b.canvasy)
                    }
                }
            }
        }
    ...
    var s = this.layout_.setNames[q];
    return{row:f, seriesName:s, point:e}
};
...
Dygraph.prototype.idxToRow_ = function (a) {
    ...
    var d = this.getLeftBoundary_();
    for (var b = 0; b < this.layout_.datasets.length; ++b) {
        var c = this.layout_.datasets[b];
        if (a < c.length) {
            return d + a
        }
        a -= c.length
...
Dygraph.prototype.updateSelection_ = function (e) {
    var m = this.cascadeEvents_("select", {selectedX:this.lastx_, selectedPoints:this.selPoints_});
    var j;
    var p = this.canvas_ctx_;
    if (this.attr_("highlightSeriesOpts")) {
        p.clearRect(0, 0, this.width_, this.height_);
        var g = 1 - this.attr_("highlightSeriesBackgroundAlpha");
        if (g) {
            var h = true;
            if (h) {
                if (e === undefined) {
            ...
            p.fillStyle = "rgba(255,255,255," + g + ")";
            p.fillRect(0, 0, this.width_, this.height_)
        }
        var d = this.datasetIndexFromSetName_(this.highlightSet_);
        this.plotter_._drawLine(p, d)
    } else {
        if (this.previousVerticalX_ >= 0) {
            var k = 0;
            var l = this.attr_("labels");
            for (j = 1; j < l.length; j++) {
                var c = this.attr_("highlightCircleSize", l[j]);
                if (c > k) {
                    k = c
                }
            }
            var n = this.previousVerticalX_;
            p.clearRect(n - k - 1, 0, 2 * k + 2, this.height_)
        }
    }
    ...
    if (this.selPoints_.length > 0) {
        ...
        for (j = 0; j < this.selPoints_.length; j++) {
            var q = this.selPoints_[j];
            if (!Dygraph.isOK(q.canvasy)) {
                continue
            }
            var a = this.attr_("highlightCircleSize", q.name);
            var o = this.attr_("drawHighlightPointCallback", q.name);
            var f = this.plotter_.colors[q.name];
            if (!o) {
                o = Dygraph.Circles.DEFAULT
            }
            p.lineWidth = this.attr_("strokeWidth", q.name);
            p.strokeStyle = f;
            p.fillStyle = f;
            o(this.g, q.name, p, b, q.canvasy, f, a)
        }
        p.restore();
        this.previousVerticalX_ = b
    }
};
Dygraph.prototype.setSelection = function (d, g) {
    this.selPoints_ = [];
    var f = 0;
    if (d !== false) {
    ...
    if (d !== false && d >= 0) {
        ...
        for (var b = 0; b < this.layout_.datasets.length; ++b) {
            ...
            if (d < e.length) {
                var a = this.layout_.points[f + d];
                if (this.attr_("stackedGraph")) {
                    a = this.layout_.unstackPointAtIndex(f + d)
                }
                if (!(a.yval === null)) {
...
Dygraph.DrawPolygon_ = function (f, c, d, b, h, e, a, g) {
    new Dygraph.RegularShape_(f, c, g).draw(d, b, h, a)
};
...
Dygraph.Interaction.startPan = function (o, t, c) {
    ...
    if (t.attr_("panEdgeFraction")) {
        var w = t.width_ * t.attr_("panEdgeFraction");
        var d = t.xAxisExtremes();
        var j = t.toDomXCoord(d[0]) - w;
        var l = t.toDomXCoord(d[1]) + w;
        var u = t.toDataXCoord(j);
        var v = t.toDataXCoord(l);
        c.boundedDates = [u, v];
        var f = [];
...
Dygraph.Interaction.movePan = function (b, k, c) {
    ...
    if (c.is2DPan) {
        for (var j = 0; j < k.axes_.length; j++) {
            ...
            var o = d * n.unitsPerPixel;
            var f = c.boundedValues ? c.boundedValues[j] : null;
            var l = n.initialTopValue + o;
            if (f) {
                l = Math.min(l, f[1])
            }
            var m = l - n.dragValueRange;
            if (f) {
                if (m < f[0]) {
                    l = l - (m - f[0]);
                    m = l - n.dragValueRange
                }
            }
            if (e.logscale) {
...
Dygraph.Interaction.moveTouch = function (b, m, c) {
    var k, l = [];
    for (k = 0; k < b.touches.length; k++) {
        var s = b.touches[k];
        l.push({pageX:s.pageX, pageY:s.pageY})
    }
    var r = c.initialTouches;
    var o;
    var a = c.initialPinchCenter;
    if (l.length == 1) {
        o = l[0]
    } else {
        o = {pageX:0.5 * (l[0].pageX + l[1].pageX), pageY:0.5 * (l[0].pageY + l[1].pageY)}
    }
    var j = {pageX:o.pageX - a.pageX, pageY:o.pageY - a.pageY};
    var f = c.initialRange.x[1] - c.initialRange.x[0];
    var q = c.initialRange.y[0] - c.initialRange.y[1];
    j.dataX = (j.pageX / m.plotter_.area.w) * f;
    j.dataY = (j.pageY / m.plotter_.area.h) * q;
    var p, d;
    if (l.length == 1) {
        p = 1;
        d = 1
        ...
        if (l.length == 2) {
            var h = (r[1].pageX - a.pageX);
            p = (l[1].pageX - o.pageX) / h;
            var n = (r[1].pageY - a.pageY);
            d = (l[1].pageY - o.pageY) / n
        }
    }
    p = Math.min(8, Math.max(0.125, p));
    d = Math.min(8, Math.max(0.125, d));
    if (c.touchDirections.x) {
        m.dateWindow_ = [a.dataX - j.dataX + (c.initialRange.x[0] - a.dataX) / p, a.dataX - j.dataX + (c.initialRange.x[1] - a.dataX) / p]
    }
    if (c.touchDirections.y) {
        for (k = 0; k < 1; k++) {
            var e = m.axes_[k];
            if (e.logscale) {
            } else {
                e.valueWindow = [a.dataY - j.dataY + (c.initialRange.y[0] - a.dataY) / d, a.dataY - j.dataY + (c.initialRange.y[1] - a.dataY) / d]
            }
        }
    }
    m.drawGraph_(false)
...
DygraphRangeSelector.prototype.initInteraction_ = function () {
    ...
    var n, e, q, i, u, g, v, s, p, b, l;
    var d, k, j;
    n = function (A) {
        var z = m.dygraph_.xAxisExtremes();
        var x = (z[1] - z[0]) / m.canvasRect_.w;
        var y = z[0] + (A.leftHandlePos - m.canvasRect_.x) * x;
        var w = z[0] + (A.rightHandlePos - m.canvasRect_.x) * x;
        return[y, w]
    };
    e = function (w) {
        ...
        r = true;
        o = w.screenX;
        t = w.target ? w.target : w.srcElement;
        m.dygraph_.addEvent(h, "mousemove", q);
        m.dygraph_.addEvent(h, "mouseup", i);
        m.fgcanvas_.style.cursor = "col-resize"
    };
    q = function (A) {
        if (!r) {
            return
        ...
        o = A.screenX;
        var z = m.getZoomHandleStatus_();
        var w;
        if (t == m.leftZoomHandle_) {
            w = z.leftHandlePos + x;
            w = Math.min(w, z.rightHandlePos - t.width - 3);
            w = Math.max(w, m.canvasRect_.x)
        } else {
            w = z.rightHandlePos + x;
            w = Math.min(w, m.canvasRect_.x + m.canvasRect_.w);
            w = Math.max(w, z.leftHandlePos + t.width + 3)
        }
        var y = t.width / 2;
        t.style.left = (w - y) + "px";
        m.drawInteractiveLayer_();
        if (f) {
            u()
        }
    };
    i = function (w) {
        ...
        r = false;
        Dygraph.removeEvent(h, "mousemove", q);
        Dygraph.removeEvent(h, "mouseup", i);
        m.fgcanvas_.style.cursor = "default";
        if (!f) {
            u()
        }
    };
    u = function () {
        try {
            var x = m.getZoomHandleStatus_();
            m.isChangingRange_ = true;
            if (!x.isZoomed) {
                m.dygraph_.doUnzoom_()
            } else {
                var w = n(x);
                m.dygraph_.doZoomXDates_(w[0], w[1])
            }
        } finally {
            m.isChangingRange_ = false
        }
    };
    g = function (y) {
        if (m.isUsingExcanvas_) {
            return y.srcElement == m.iePanOverlay_
        } else {
            var x = m.leftZoomHandle_.getBoundingClientRect();
            var w = x.left + x.width / 2;
            x = m.rightZoomHandle_.getBoundingClientRect();
            var z = x.left + x.width / 2;
            return(y.clientX > w && y.clientX < z)
        }
    };
    v = function (w) {
        if (!c && g(w) && m.getZoomHandleStatus_().isZoomed) {
            Dygraph.cancelEvent(w);
            c = true;
            o = w.screenX;
            m.dygraph_.addEvent(h, "mousemove", s);
            m.dygraph_.addEvent(h, "mouseup", p)
        }
    };
    s = function (A) {
        if (!c) {
        ...
        o = A.screenX;
        var z = m.getZoomHandleStatus_();
        var C = z.leftHandlePos;
        var w = z.rightHandlePos;
        var B = w - C;
        if (C + x <= m.canvasRect_.x) {
            C = m.canvasRect_.x;
            w = C + B
        } else {
            if (w + x >= m.canvasRect_.x + m.canvasRect_.w) {
                w = m.canvasRect_.x + m.canvasRect_.w;
                C = w - B
            } else {
                C += x;
                w += x
        ...
        var y = m.leftZoomHandle_.width / 2;
        m.leftZoomHandle_.style.left = (C - y) + "px";
        m.rightZoomHandle_.style.left = (w - y) + "px";
        m.drawInteractiveLayer_();
        if (f) {
            b()
        }
    ...
    p = function (w) {
        ...
        Dygraph.removeEvent(h, "mouseup", p);
        if (!f) {
            b()
        }
    };
    b = function () {
        try {
            m.isChangingRange_ = true;
            m.dygraph_.dateWindow_ = n(m.getZoomHandleStatus_());
            m.dygraph_.drawGraph_(false)
        } finally {
            m.isChangingRange_ = false
        }
    };
    l = function (w) {
        ...
        var x = g(w) ? "move" : "default";
        if (x != m.fgcanvas_.style.cursor) {
            m.fgcanvas_.style.cursor = x
        }
    };
    d = function (w) {
        w.preventDefault();
    ...
    j = function (z, y) {
        var x = ["touchstart", "touchend", "touchmove", "touchcancel"];
        for (var w = 0; w < x.length; w++) {
            m.dygraph_.addEvent(z, x[w], y)
        }
    };
    ...
    if (this.isUsingExcanvas_) {
        this.dygraph_.addEvent(this.iePanOverlay_, "mousedown", v)
    } else {
        this.dygraph_.addEvent(this.fgcanvas_, "mousedown", v);
        this.dygraph_.addEvent(this.fgcanvas_, "mousemove", l)
    }
    if (this.hasTouchInterface_) {
        j(this.leftZoomHandle_, d);
        j(this.rightZoomHandle_, d);
        j(this.fgcanvas_, k)
    }
};
...
Dygraph.numericTicks = function (I, H, w, r, d, s) {
    ...
    for (F = 0; F < J.length; F++) {
        ...
        var l = E(v, 0, r, d);
        if (y.length > 0) {
            var x = Math.pow(B, y.length);
            for (D = y.length - 1; D >= 0; D--, x /= B) {
                if (e >= x) {
...
Dygraph.Plugins.Legend = (function () {
    ...
    c.prototype.select = function (j) {
        var i = j.selectedX;
        var h = j.selectedPoints;
        var f = b(this.legend_div_);
        var g = a(j.dygraph, i, h, f);
    ...
    c.prototype.drawChart = function (f) {
        this.deselect(f)
    };
    c.prototype.predraw = function (g) {
