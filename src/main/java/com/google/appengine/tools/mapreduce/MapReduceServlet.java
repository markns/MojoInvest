...
public class MapReduceServlet extends HttpServlet {
  ...
  /**
   * MapperFactory class which is used for the instantiation of the mapper classes. The factory is lazily initialized in the
   * {@link com.google.appengine.tools.mapreduce.MapReduceServlet#getMapperFactory()} method.
   */
  ...
  void handleMapperWorker(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      AppEngineMapper<INKEY,INVALUE,OUTKEY,OUTVALUE> mapper = getMapperFactory()
              .createMapper(
                      taskAttemptContext.getMapperClass(),
  ...
  protected MapperFactory getMapperFactory() {
    if (mapperFactory == null) {
      mapperFactory = new ReflectionMapperFactory();
    }
  ...
  public void handleStaticResources(String handler, HttpServletResponse response) {
    ...
    try {
      InputStream resourceStream = MapReduceServlet.class.getResourceAsStream(
          "/com/google/appengine/tools/mapper/" + fileName);
